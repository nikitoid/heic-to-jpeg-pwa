...
// --- ИЗМЕНЕНО: Создаем заглушку для объекта window ---
// Библиотека heic2any пытается получить доступ к 'window', которого нет в воркерах.
// Создаем self.window, чтобы обмануть библиотеку, так как 'self' является глобальным объектом в воркере.
self.window = self;
